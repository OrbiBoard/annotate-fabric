...
(async () => {
  ...
  if (!fabric || typeof fabric.Canvas !== 'function') {
    ...
    let ok = false;
    for (const p of candidates) { try { await loadScript(p); ok = true; break; } catch (e) {} }
  ...
  try { canvas.setBackgroundColor('#121212', () => { if (canvas.requestRenderAll) canvas.requestRenderAll(); }); } catch (e) {}
  ...
  if (m === 'draw') {
    ...
    if (!(currentBrush instanceof fabric.PencilBrush)) {
      try { currentBrush = new fabric.PencilBrush(canvas); } catch (e) { try { currentBrush = new fabric.PencilBrush({ canvas }); } catch (e) {} }
    ...
    hidePopup();
      ...
      let eBrush = null;
      try { if (fabric && fabric.EraserBrush) eBrush = new fabric.EraserBrush(canvas); } catch (e) {}
      if (eBrush) {
  ...
  let brush = null;
  try { brush = new fabric.PencilBrush(canvas); } catch (e) { try { brush = new fabric.PencilBrush({ canvas }); } catch (e) {} }
  ...
  paletteBtns.forEach((b) => {
    b.addEventListener('click', () => {
      ...
      if (!c) return;
      penColor.value = c;
      try { if (canvas.freeDrawingBrush) canvas.freeDrawingBrush.color = c; } catch (e) {}
  ...
  penWidthButtons.forEach((b) => {
    b.addEventListener('click', () => {
      ...
      try { if (state.mode === 'draw' && canvas.freeDrawingBrush) canvas.freeDrawingBrush.width = v; } catch (e) {}
    });
  });
  ...
  if (btnClose) btnClose.addEventListener('click', () => { try { if (window.annotateAPI && typeof window.annotateAPI.close === 'function') { window.annotateAPI.close(); } else { window.close(); } } catch (e) {} });
  ...
  canvas.on('path:created', (e) => {
    try {
      ...
      try { if (state.mode === 'erase' && !history.restoring) history.recordHistory(); } catch (e) {}
    } catch (e) {}
  });
  ...
  function savePageSnapshot() {
    try {
      ...
      try { const url = canvas.toDataURL({ format: 'png', multiplier: 0.2 }); state.pageThumbs[state.pageIndex] = url; } catch (e) {}
    } catch (e) {}
  }
  ...
  if (data) {
    canvas.loadFromJSON(data, () => {
      try { setBackgroundColor(state.bgColor, true); } catch (e) {}
      canvas.renderAll();
      setMode(state.mode);
      ...
      if (state.mode === 'erase') {
          try {
            ...
            let eBrush = null;
            try { eBrush = new fabric.EraserBrush(canvas); } catch (e) {}
            if (eBrush) {
              eBrush.width = Number(state.eraseSize) || 80;
              eBrush.strokeLineCap = 'round';
              eBrush.strokeLineJoin = 'round';
    ...
    updatePageLabel();
    try { if (!state.pageThumbs[i]) { const url = canvas.toDataURL({ format: 'png', multiplier: 0.2 }); state.pageThumbs[i] = url; } } catch (e) {}
    savePackIfNeeded();
  }
  ...
  loadPageSnapshot(0);
  setInterval(() => { try { savePageSnapshot(); savePackIfNeeded(); } catch (e) {} }, 3000);
  ...
  document.getElementById('importFile').addEventListener('change', (e) => { const f = e.target.files && e.target.files[0]; if (!f) return; const reader = new FileReader(); reader.onload = () => { try { const data = JSON.parse(String(reader.result || '{}')); canvas.clear(); canvas.loadFromJSON(data, () => { canvas.renderAll(); setMode(state.mode); }); } catch (e) {} }; reader.readAsText(f); });
    ...
    if (dbg === '1') {
      setTimeout(() => {
        try {
          ...
          canvas.add(r);
          if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll();
          try { history.recordHistory(); } catch (e) {}
          r.left = 200; r.top = 140; r.setCoords();
          try { canvas.fire('object:modified', { target: r }); } catch (e) {}
          if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll();
          try { history.undo(); } catch (e) {}
          setTimeout(() => { try { history.redo(); } catch (e) {} }, 100);
        } catch (e) {}
      }, 1000);
    }
  } catch (e) {}
  ...
  canvas.upperCanvasEl.addEventListener('touchend', (e) => { draggingPan = false; if ((state.mode === 'draw' || state.mode === 'erase') && !history.restoring) { try { history.recordHistory(); } catch (e) {} } history.updateUndoRedoUI(); }, { passive: false });
  ...
  function setBackgroundColor(color, skipHistory = false) {
    state.bgColor = color;
    try { canvas.setBackgroundColor(color, () => { updateBgButtonsActive(); if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll(); if (!skipHistory) { try { history.recordHistory(); } catch (e) {} } }); }
    catch (e) { canvas.backgroundColor = color; updateBgButtonsActive(); if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll(); if (!skipHistory) { try { history.recordHistory(); } catch (e) {} } }
  }
  ...
  async function genThumbForPage(i, imgEl) {
    try {
      ...
      const el = document.createElement('canvas');
      let sc = null; try { sc = new fabric.StaticCanvas(el); } catch (e) { sc = null; }
  ...
  canvas.on('object:modified', () => { if (!history.restoring) { try { history.recordHistory(); } catch (e) {} } history.updateUndoRedoUI(); });
  async function cloneObject(o) { try { if (!o) return null; let target = o; if (target.type === 'activeSelection' && typeof target.toGroup === 'function') { try { target = target.toGroup(); } catch (e) {} } if (typeof target.clone !== 'function') return null; if (target.clone.length > 0) { return await new Promise((resolve) => { try { target.clone((cl) => resolve(cl)); } catch (e) { resolve(null); } }); } else { const res = target.clone(); if (res && typeof res.then === 'function') { return await res; } return res || null; } } catch (e) { return null; } }
  ...
  selTools.querySelector('#toolDelete').addEventListener('click', () => { const o = canvas.getActiveObject(); if (!o) return; canvas.remove(o); if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll(); hideSelTools(); try { history.recordHistory(); } catch (e) {} });
  ...
  clearPageBtn.addEventListener('click', () => { const bg = canvas.backgroundColor; canvas.clear(); if (bg) setBackgroundColor(bg); setMode('draw'); state.pages[state.pageIndex] = null; if (canvas.requestRenderAll) canvas.requestRenderAll(); else canvas.renderAll(); try { hideSelTools(); } catch (e) {} try { hidePopup(); } catch (e) {} try { updateEraseCursorVis(); } catch (e) {} try { history.recordHistory(); } catch (e) {} });
})();
